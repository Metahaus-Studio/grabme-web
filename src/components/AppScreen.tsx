...
export function AppScreen({ type }: AppScreenProps) {
  ...
  return (
    <div className="relative h-[620px] w-[310px] overflow-hidden rounded-[3rem] border border-white/15 bg-[#0B0F0D] p-4 shadow-2xl shadow-[#7AC943]/10">
          ...
          {[
            ...
            <div
              ...
              className={`flex items-center justify-between rounded-2xl p-3 ${
                index === 1 ? "border border-[#7AC943] bg-[#7AC943]/10" : "bg-white/5"
              }`}
            >
      ...
      {[
        ...
        <div
          ...
          className={`rounded-3xl border p-4 ${
            index === 1 ? "border-[#7AC943] bg-[#7AC943]/10" : "border-white/10 bg-white/5"
          }`}
        >
      ...
      <div className="absolute bottom-5 left-4 right-4 z-10 rounded-[2rem] border border-white/10 bg-black/80 p-5 backdrop-blur-xl">
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          {["Call", "Share", "Cancel"].map((label) => (
            <button key={label} className="rounded-2xl bg-white/5 py-3 text-xs font-bold text-white/80">
              {label}
            </button>
          ))}
        </div>
...
function AirportScreen() {
  return (
    <div className="relative z-10 mt-8">
      ...
      <div className="mt-4 space-y-3">
        {["Terminal pickup", "Luggage-friendly car", "Live driver tracking"].map((item) => (
          <div key={item} className="rounded-2xl bg-white/5 p-4 text-sm font-semibold text-white/75">
            {item}
          </div>
        ))}
      </div>
  ...
  return (
    <div className="relative z-10 mt-8">
      <div className="rounded-[2rem] border border-[#7AC943]/30 bg-[#7AC943]/10 p-5">
        ...
        <p className="mt-2 text-sm text-white/55">Student pricing available</p>
      </div>

      <div className="mt-4 grid gap-3">
        {["University routes", "Group mobility", "Membership discount"].map((item) => (
          <div key={item} className="rounded-2xl bg-white/5 p-4 text-sm font-semibold text-white/75">
            {item}
          </div>
        ))}
  ...
  return (
    <div className="relative z-10 mt-8">
      <div className="rounded-[2rem] border border-white/10 bg-white/5 p-5">
        ...
        <p className="mt-2 text-sm text-white/55">Redeem for credits, gifts, and priority rides.</p>
      </div>
      ...
      <div className="mt-4 space-y-3">
        {[
          ...
          <div key={name} className="flex items-center justify-between rounded-2xl bg-white/5 p-4">
            <div>
